...
Таким образом, мы вычислили выражение `8 - (-2 + 17) = -7`. Это можно проллюстрировать таблицей:
...
- Для упрощения разбора команд мы применим возможности функций scanf/sscanf/fscanf.
...
- Чтобы сделать функцию `execute` более гибкой, передадим в неё `FILE*`. Для чтения с консоли достаточно будет вызвать функцию с параметром `stdin`, пример:  `execute(stdin);`
...
В нашем случае scanf может вернуть один из 4-х значений: EOF, 0, 1 или 2.
...
- значения 0, 1 и 2 показывают, сколько аргументов `scanf` удалось успешно распарсить и записать
- есть нюанс: при тестировании приложения в консоли scanf будет ожидать ввода целого числа и из-за этого отставать на 1 аргумент. То есть, после ввода "push 10 pop" команда pop не будет прочитана, пока не появится конец файла или следующая строка. При чтении из файла такого не будет.
