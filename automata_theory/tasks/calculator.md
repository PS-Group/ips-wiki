...
- вместо ```boost::string_ref``` можно использовать экспериментальный ```std::string_view```. Этот компонент STL пока реализован не во всех компиляторах, работоспособная замена есть в [репозитории на github](https://github.com/sergey-shambir/string_view).

Дополнительные требования (на повышенную оценку):
- поддержка скобок
- поддержка функций sin, cos, sqrt, rand(min, max).
- разбор [научной формы](https://en.wikipedia.org/wiki/Scientific_notation) чисел с плавающей запятой, например: ```1.25e-7```
...
В примере остановимся на ```boost::string_ref```.
Вот так будет выглядеть функция для разбора чисел вида "124", "15", "012":
...
Заметим, что мы не можем использовать функцию [std::stof](http://en.cppreference.com/w/cpp/string/basic_string/stof), т.к. она не обрабатывает тип данных ```boost::string_ref```, а нам нужен именно string_ref для последовательного движения по выражению слева направо.
...
Чтобы сделать функцию универсальной, мы можем выбирать дальнейшее действие по очередному символу. Пример ниже должен вывести ```100```.
